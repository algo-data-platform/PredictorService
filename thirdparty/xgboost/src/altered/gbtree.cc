...
class Dart : public GBTree {
 public:
  ...
  void PredictInstance(const SparsePage::Inst& inst,
    ...
    local_thread_temp[0].Init(model_.param.num_feature);
    // -------------------------------------------------
    ...
    for (int gid = 0; gid < model_.param.num_output_group; ++gid) {
      ...
      (*out_preds)[gid]
          = PredValue(inst, gid, root_index,
                      &local_thread_temp[0], 0, ntree_limit) + model_.base_margin;
    }
  }
 ...
 protected:
  ...
  inline void PredLoopSpecalize(
    ...
    std::vector<RegTree::FVec> local_thread_temp;
    InitThreadTempSafe(nthread, &local_thread_temp);
    // -------------------------------------------------
  ...
  // extra: init thread input buffers, for thread safe
  inline void InitThreadTempSafe(const int& nthread, std::vector<RegTree::FVec>* temp_vec) {
    int prev_thread_temp_size = temp_vec->size();
    if (prev_thread_temp_size < nthread) {
      temp_vec->resize(nthread, RegTree::FVec());
      for (int i = prev_thread_temp_size; i < nthread; ++i) {
        (*temp_vec)[i].Init(model_.param.num_feature);
      }
    }
  }
  // -------------------------------------------------
