...
class CPUPredictor : public Predictor {
 protected:
  ...
  // extra: init thread buffers for thread safe
  inline void InitThreadTempSafe(const int& nthread, const int& num_feature, std::vector<RegTree::FVec>* temp_vec) {
    int prev_thread_temp_size = temp_vec->size();
    if (prev_thread_temp_size < nthread) {
      temp_vec->resize(nthread, RegTree::FVec());
      for (int i = prev_thread_temp_size; i < nthread; ++i) {
        (*temp_vec)[i].Init(num_feature);
      }
    }
  }
  // -------------------------------------------------
  inline void PredLoopSpecalize(DMatrix* p_fmat,
    ...
    InitThreadTempSafe(nthread, model.param.num_feature, &local_thread_temp);
    // -------------------------------------------------
...
#pragma omp parallel for schedule(static)
 ...
 public:
  ...
  void PredictInstance(const SparsePage::Inst& inst,
    ...
    // extra: for thread safe
    std::vector<RegTree::FVec> local_thread_temp;
    local_thread_temp.resize(1, RegTree::FVec());
    local_thread_temp[0].Init(model.param.num_feature);
    // -------------------------------------------------
    ...
    for (int gid = 0; gid < model.param.num_output_group; ++gid) {
      ...
      (*out_preds)[gid] =
          PredValue(inst, model.trees, model.tree_info, gid, root_index,
                    &local_thread_temp[0], 0, ntree_limit) +
          model.base_margin;
      // -------------------------------------------------
    }
  }
...
#pragma omp parallel for schedule(static)
      for (bst_omp_uint i = 0; i < nsize; ++i) {
        ...
        RegTree::FVec& feats = local_thread_temp[omp_get_thread_num()];
        // -------------------------------------------------
