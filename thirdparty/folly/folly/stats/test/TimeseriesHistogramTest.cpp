...
TEST(TimeseriesHistogram, Percentile) {
  ...
  // [10, 109], 12 buckets including above and below
  {
    ...
    // In the histogram, the bucket size is 10, the max value is 120 and the min value is
    //  0 ( after h.addValue() )
    // As defined in the TimeseriesHistogram.h, the getPercentileEstimate() function
    //  returns an estimate of the value at the given percentile in the histogram.
    //  So, according to the estimation algorithm, the 1 percentile value should be 0.
    EXPECT_EQ(-2, h.getPercentileEstimate(0, IntMTMHTS::ALLTIME));
    EXPECT_EQ(0, h.getPercentileEstimate(1, IntMTMHTS::ALLTIME));
