...
static std::string getReqIds(const std::vector<predictor::PredictRequest> &reqs) {
  std::string req_ids;
...
static std::string getReqIds(const std::vector<predictor::CalculateVectorRequest> &reqs) {
  std::string req_ids;
  for (const auto& req : reqs) {
    req_ids += req.get_req_id() + " ";
  }
...
static void markModelMeter(const std::string &metric_name, const std::string &model_name,
  ...
  metrics::Metrics::getInstance()->buildMeter(SDK_MODULE_NAME, metric_name, tags_map)->mark(num);
}
// service level meter
...
static void markMeters(const std::string &metric_name, const std::vector<predictor::PredictRequest> &reqs,
...
static void markMeters(const std::string &metric_name, const std::vector<predictor::CalculateVectorRequest> &reqs,
                       const std::string &service_name, const std::string &request_type = PREDICT_REQUEST) {
  for (const auto& req : reqs) {
    markModelMeter(metric_name, req.get_model_name(), 1, request_type);
    markChannelMeter(metric_name, req.get_channel(), 1, request_type);
...
static void markModelAsyncRequestMetrics(const std::string &model_name, int num = 1,
                                  const std::string& request_type = PREDICT_REQUEST) {
  const MetricTagsMap tags_map{{TAG_MODEL, model_name}, {REQUEST_TYPE, request_type}};
  metrics::Metrics::getInstance()->buildMeter(SDK_MODULE_NAME, ASYNC_REQ_METER, tags_map)->mark(num);
}
...
static void markModelAsyncErrorMetrics(const std::string &model_name, int num = 1,
                                const std::string& request_type = PREDICT_REQUEST) {
  const MetricTagsMap tags_map{{TAG_MODEL, model_name}, {REQUEST_TYPE, request_type}};
  metrics::Metrics::getInstance()->buildMeter(SDK_MODULE_NAME, ASYNC_ERROR_METER, tags_map)->mark(num);
}
...
static void markModelAsyncGetExceptionMetrics(const std::string &model_name, int num = 1,
                                       const std::string& request_type = PREDICT_REQUEST) {
  const MetricTagsMap tags_map{{TAG_MODEL, model_name}, {REQUEST_TYPE, request_type}};
  metrics::Metrics::getInstance()->buildMeter(SDK_MODULE_NAME, ASYNC_GET_EXCEPTION_METER, tags_map)->mark(num);
}
...
static void markModelAsyncGetEmptyResponseMetrics(const std::string &model_name, int num = 1,
                                           const std::string& request_type = PREDICT_REQUEST) {
  const MetricTagsMap tags_map{{TAG_MODEL, model_name}, {REQUEST_TYPE, request_type}};
  metrics::Metrics::getInstance()->buildMeter(SDK_MODULE_NAME, ASYNC_GET_EMPTY_RESP_METER, tags_map)->mark(num);
}
...
static void markModelSyncRequestMetrics(const std::string &model_name, int num = 1,
  ...
  metrics::Metrics::getInstance()->buildMeter(SDK_MODULE_NAME, SYNC_REQ_METER, request_meter_tag)->mark(num);
}
...
static void markModelSyncRequestErrorMetrics(const std::string &model_name, int num = 1,
  ...
  metrics::Metrics::getInstance()->buildMeter(SDK_MODULE_NAME, SYNC_PREDICT_ERROR, err_tag)->mark(num);
}
...
static void markModelSyncRequestEmptyMetrics(const std::string &model_name, int num = 1,
                                      const std::string& request_type = PREDICT_REQUEST) {
  const MetricTagsMap err_tag{{TAG_MODEL, model_name}, {REQUEST_TYPE, request_type}};
  metrics::Metrics::getInstance()->buildMeter(SDK_MODULE_NAME, SYNC_PREDICT_EMPTY, err_tag)->mark(num);
}
