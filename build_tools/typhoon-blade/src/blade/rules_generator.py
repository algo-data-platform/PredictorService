...
# import commands to use shell cmd @niuqing 
import commands
...
""")
    ...
    def _generate_fbthrift_builders(self):
        ...
        fbthrift_bin = fbthrift_config['fbthrift']
        fbthrift_py = fbthrift_config['fbthrift_py']
        fbthrift_templates = fbthrift_config['fbthrift_templates']
        ...
        self._add_rule('scons_helper.setup_fbthrift_builders(top_env, "%s", '
                'fbthrift_bin="%s", fbthrift_templates="%s", fbthrift_incs_str="%s", fbthrift_py="%s")' % (
                    self.build_dir, fbthrift_bin, fbthrift_templates, fbthrift_incs_str, fbthrift_py))
...
# makefile header generator @niuqing
class MakeScriptHeaderGenerator(ScriptHeaderGenerator):
    """Genearte Makefile header"""
    def __init__(self, options, build_dir, blade_path, gcc_version,
                 python_inc, cuda_inc, build_environment, svn_roots):
        ScriptHeaderGenerator.__init__(
                self, options, build_dir, gcc_version,
    ...
    def generate_rule(self, target, prereq, cmd):
        pass

    def generate_makefile_env_info(self):
        self._add_rule('''
#------------------#
# blade_path = %s
# build_dir = %s
...
# time = %s
#------------------#
''' % (self.blade_path, self.build_dir, self.options.profile, 'GCC ' + self.gcc_version, sorted(self.svn_roots), time.asctime()))

    # shell command @niuqing
    def generate_makefile_compliation_env(self):
        gcc_current = 'which gcc'
        gpp_cmd = 'which g++'
        clang_cmd = 'which clang'
        ar_cmd = 'which ar'

        gcc_current_tup = commands.getstatusoutput(gcc_current) 
        gpp_tup = commands.getstatusoutput(gpp_cmd)
        clang_tup = commands.getstatusoutput(clang_cmd)
        ar_tup = commands.getstatusoutput(ar_cmd)

        self._add_rule("# change CC if necessary")
        self._add_rule("CC = %s" % gcc_current_tup[1])
        self._add_rule("# change CXX if necessary")
        self._add_rule("CXX = %s" % gpp_tup[1])
        self._add_rule("# change CLANG if necessary")
        self._add_rule("CLANG = %s" % clang_tup[1])
        self._add_rule("# change AR if necessary")
        self._add_rule("AR = %s" % ar_tup[1])
        self._add_rule("# mkdir command in Makefile")
        self._add_rule("MKDIR = mkdir -p")
        self._add_rule("""
...
""")
    ...
    # generate flags @niuqing
    def generate_makefile_compliation_flags(self):
        """Generates compliation flags. """
        toolchain_dir = os.environ.get('TOOLCHAIN_DIR', '')
        if toolchain_dir and not toolchain_dir.endswith('/'):
            toolchain_dir += '/'
        ...
        extra_incs = cc_config['extra_incs']
        extra_incs_str = ' '.join(['-I%s' % inc for inc in extra_incs])
        if not extra_incs_str:
            extra_incs_str = '""'
        ...
        self._add_rule("# CC = %s" % cc_str)
        self._add_rule("# CXX = %s" % cxx_str)
        self._add_rule("# SECURECXX = %s %s" % (cc_config['securecc'], cxx))
        self._add_rule("CPPPATH = %s -I%s -I. -I%s"
                        %(extra_incs_str, self.build_dir, self.python_inc))
        self._add_rule("%s" % ld_env_str)
        self._add_rule("LINKFLAGS = %s" % (" ".join(linkflags)))
        self._add_rule("CPPFLAGS = %s" % (" ".join(cc_config['cppflags'] + cppflags_except_warning)))
        self._add_rule("CFLAGS = %s" % (" ".join(cc_config['cflags'])))
        self._add_rule("CXXFLAGS = %s" % (" ".join(cc_config['cxxflags'])))
        self._add_rule("COMPILE_FLAGS = -m64 -static-libgcc -static-libstdc++")
        
        cc_library_config = self.blade_config.get_config('cc_library_config')
        # By default blade use 'ar rcs' and skip ranlib
        # to generate index for static library
        arflags = ''.join(cc_library_config['arflags'])
        self._add_rule('ARFLAGS = %s' % arflags)

        # The default ASPPFLAGS  is same as ASFLAGS,
        # this is incorrect for gcc/gas
        options = self.options
        self._add_rule('ASFLAGS = -g --%s' % options.m)
        self._add_rule('ASPPFLAGS = -Wa --%s' % options.m)

        self._setup_makefile_cache()
        self._setup_env_cc()

    def _setup_makefile_cache(self):
        self.build_environment.setup_build_cache(self.options)

    def _setup_env_cc(self):
        warnings, cxx_warnings, c_warnings = self.ccflags_manager.get_warning_flags()
        self._add_rule('WARNINGS_CPPFLAGS = %s' % (" ".join(warnings)))
        self._add_rule('WARNINGS_CFLAGS = %s' % (" ".join(c_warnings)))
        self._add_rule('WARNINGS_CXXFLAGS = %s' % (" ".join(cxx_warnings)))

    def generate_makefile_header_info(self):
        self._add_rule('# Makefile generated by blade')
   
    def generate_makefile_phony(self):
        self._add_rule('.PHONY: clean default all test all_test lint dbg_test opt_test')

    def generate(self):
        """Generate make rules. """
        #self.generate_rule() 
        self.generate_makefile_header_info()
        self.generate_makefile_env_info() 
        self.generate_makefile_compliation_env()
        self.generate_makefile_compliation_flags()
        self.generate_makefile_phony()
        return self.rules_buf
...
# subclass of RulesGenerator @niuqing
class MakeRulesGenerator(RulesGenerator):
    """The main class to generate make rules and outputs rules to Makefile. """
    def __init__(self, make_path, blade_path, blade):
        RulesGenerator.__init__(self, make_path, blade_path, blade)
        options = self.blade.get_options()
        gcc_version = self.scons_platform.get_gcc_version()
        python_inc = self.scons_platform.get_python_include()
        cuda_inc = self.scons_platform.get_cuda_include()
        self.make_script_header_generator = MakeScriptHeaderGenerator(
                options,
                self.build_dir,
                self.blade_path, 
                gcc_version,
    ...
    def generate_build_rules(self):
        """Generates make rules to Makefile. """
        rules = self.make_script_header_generator.generate()
        rules += self.blade.gen_targets_rules()
        return rules
