...
class FBThriftLibrary(CcTarget):
	...
	include_prefix_text = 'include_prefix=%s' % include_prefix
	if options != '':
	    include_prefix_text = ',%s' % include_prefix_text

	     
	self.data['options'] = '%s%s' % (options, include_prefix_text)
    ...
    def scons_rules(self):
  	...
  	fb_flags = []
        fb_flags.append(self.data['options'])
        self._write_rule('%s.Replace(FBOPTIONS=%s)' % (env_name, fb_flags))
