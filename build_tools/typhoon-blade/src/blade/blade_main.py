...
# add make build @niuqing
def _make_build(options):
    cmd = ['make']
    cmd += native_builder_options(options)
    cmd.append('-j%s' % (options.jobs or blade.blade.parallel_jobs_num()))
    if options.keep_going:
...
def build(options):
    ...
    # add _make_build @niuqing
    console.info('building...')
    console.flush()
    if configparse.blade_config.get_config('global_config')['native_builder'] == 'ninja':
        returncode = _ninja_build(options)
    elif configparse.blade_config.get_config('global_config')['native_builder'] == 'make':    
        returncode = _make_build(options)
    else:
...
def clean(options):
    ...
    cmd = [native_builder]
    # add make in if..elif..else @niuqing
    # cmd += native_builder_options(options)
...
def clear_build_script():
    # add Makefile @niuqing
    for script in ('SConstruct', 'build.ninja', 'Makefile'):
