...
class Target(object):
    ...
    # add make_rules() @niuqing
    def make_rules(self):
        """Generate make rules for specific target. """
        raise NotImplementedError(self.fullname)

    # implement make_build() @niuqing
    def make_build(self, outputs, rule, inputs=None,
                    implicit_deps=None, order_only_deps=None,
                    variables=None):
        """Generate a make build statement with specified parameters. """
        outs = var_to_list(outputs)
        ins = []
        if inputs:
            ins = var_to_list(inputs)
        ...
        if variables:
            ...
            deps_str = json.dumps(variables.get('includes'))
            # strip " in the json string
            deps = deps_str.strip().strip('"')

            cpp_flags = json.dumps(variables.get('cppflags'))
            flags = cpp_flags.strip().strip('"') 
        
        # create folder for .o targets
        seperator = '/'
        mkdir_path = (' '.join(outs).rpartition(seperator))[0]
        # add clean rule and ar rule
        if rule == 'clean':
            self._write_rule('rm -rf ${TARGET} ${TARGET}.objs/*.a ${TARGET}.objs/*.o \n')
            self._write_rule('clean : ; \\')
        elif rule == 'static_lib':
            self._write_rule('${AR} ${ARFLAGS} %s %s \n' % (' '.join(outs), ' '.join(ins)))
            self._write_rule('%s : %s; \\' % (' '.join(outs), ' '.join(ins)))
        elif rule == 'object' or rule == 'securecc' or rule == 'securecccompile':
            if deps == 'null' and flags == 'null':
                self._write_rule('${CXX} -o %s -c ${CXXFLAGS} ${WARNINGS_CXXFLAGS} ${CPPFLAGS} ${WARNINGS_CPPFLAGS} ${CPPPATH} %s \n' % (' '.join(outs), ' '.join(ins), ' '.join(ins)))
                self._write_rule('${MKDIR} %s; \\' % mkdir_path) 
                self._write_rule('%s : %s; \\' % (' '.join(outs), ' '.join(ins)))
            elif deps != 'null' and flags == 'null':
                self._write_rule('${CXX} -o %s -c ${CXXFLAGS} ${WARNINGS_CXXFLAGS} ${CPPFLAGS} ${WARNINGS_CPPFLAGS} %s ${CPPPATH} %s \n' % (' '.join(outs), deps, ' '.join(ins)))
                self._write_rule('${MKDIR} %s; \\' % mkdir_path)
                self._write_rule('%s : %s; \\' % (' '.join(outs), ' '.join(ins)))
            elif deps == 'null' and flags != 'null':
                self._write_rule('${CXX} -o %s -c ${CXXFLAGS} ${WARNINGS_CXXFLAGS} ${CPPFLAGS} ${WARNINGS_CPPFLAGS} %s ${CPPPATH} %s \n' % (' '.join(outs), flags, ' '.join(ins)))
                self._write_rule('${MKDIR} %s; \\' % mkdir_path)
                self._write_rule('%s : %s; \\' % (' '.join(outs), ' '.join(ins)))
            else:
                self._write_rule('${CXX} -o %s -c ${CXXFLAGS} ${WARNINGS_CXXFLAGS} ${CPPFLAGS} ${WARNINGS_CPPFLAGS} %s %s ${CPPPATH} %s \n' % (' '.join(outs), flags, deps, ' '.join(ins)))
                self._write_rule('${MKDIR} %s; \\' % mkdir_path) 
                self._write_rule('%s : %s; \\' % (' '.join(outs), ' '.join(ins)))
        elif rule == 'link':   
            if deps == 'null':
                self._write_rule('${CXX} -o %s ${COMPILE_FLAGS} %s -lpthread\n' % (' '.join(outs), ' '.join(ins)))
                self._write_rule('${TARGET} : %s; \\' % ' '.join(ins))
                self._write_rule('all : ${TARGET} \n')
                self._write_rule('TARGET = %s \n' % ' '.join(outs))
            else:
                self._write_rule('${CXX} -o %s ${COMPILE_FLAGS} %s -lpthread\n' % (' '.join(outs), ' '.join(ins)))
                self._write_rule('${TARGET} : %s %s ; ${CXX} -o %s ${COMPILE_FLAGS} %s -lpthread \\' % (' '.join(ins), deps))
                self._write_rule('all : ${TARGET} \n')
                self._write_rule('TARGET = %s \n' % ' '.join(outs))
