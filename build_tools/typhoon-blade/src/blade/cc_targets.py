...
class CcTarget(Target):
    ...
    def _prebuilt_cc_library(self):
        ...
        # add native_builder == make @niuqing
        if self.blade.get_config('global_config')['native_builder'] == 'ninja':
            paths = self._prebuilt_cc_library_ninja_rules()
        elif self.blade.get_config('global_config')['native_builder'] == 'make':
            paths = self._prebuilt_cc_library_make_rules()
        else:
    ...
    # ninja header files dependency, commented by @niuqing
    ...
    # functions to generate make rules @niuqing
    # TODO
    # add _prebuilt_cc_library_make_rules() @niuqing
    def _prebuilt_cc_library_make_rules(self):
        """Prebuilt cc library make rules. """
        static_src_path, static_target_path = self._prebuilt_cc_library_path()
        if static_src_path.endswith('.a'):
            path = static_src_path
        else:
            self.make_build(static_target_path, 'copy',
                             inputs=static_src_path)
            path = static_target_path
        self._add_default_target_file('a', path)
        ...
        if self._need_dynamic_library():
            ...
            if dynamic_target_path != static_target_path:
                assert static_src_path.endswith('.a')
                assert dynamic_src_path.endswith('.so')
                self.make_build(dynamic_target_path, 'copy',
                                 inputs=dynamic_src_path)
                path = dynamic_target_path
            self._add_target_file('so', path)
    ...
    # add _setup_make_cc_vars(), _securecc_object_make(), implement _cc_objects_make()  @niuqing
    def _setup_make_cc_vars(self, vars):
        """Set up warning, compile options and include directories for cc build. """
        if self.data.get('warning') != 'yes':
            vars['c_warnings'] = ''
            vars['cxx_warnings'] = ''
    ...
    # add _setup_make_cc_flags() @niuqing
    def _setup_make_cc_flags(self):
        """_setup_cc_flags. """
        env_name = self._env_name()
        flags_from_option, incs_list = self._get_cc_flags()
        if flags_from_option:
            flags = ' '.join(['%s' % inc for inc in flags_from_option])
            self._write_rule('# %sCPPFLAGS = %s\n' % (self.name.upper(), flags))
        if incs_list:
            path = ' '.join(['-I%s' % inc for inc in incs_list])
            self._write_rule('# %sCPPPATH = %s\n' % (self.name.upper(), path))

    def _securecc_object_make(self, obj, src, implicit_deps, vars):
        assert obj.endswith('.o')
        pos = obj.rfind('.', 0, -2)
        assert pos != -1
        secure_obj = '%s.cc.o' % obj[:pos]
        path = self._source_file_path(src)
        if not os.path.exists(path):
            path = self._target_file_path(src)
            self._securecc_object_rules('', path, False)
        self.make_build(secure_obj, 'securecccompile', inputs=path,
                         implicit_deps=implicit_deps,
                         variables=vars)
        self.make_build(obj, 'securecc', inputs=secure_obj)


    # make header files dependency @niuqing
    def _cc_objects_generated_make_header_files_dependency(self):
        """Return a stamp which depends on targets which generate header files. """
        deps = self._generated_header_files_dependencies()
        if not deps:
            return None
        stamp = self._target_file_path('%s' % self.name)
        inputs = []
        for dep in deps:
            dep_output = dep._get_target_file()
            if dep_output:
                inputs.append(dep_output)
        if inputs != []:
            self.make_build(stamp, 'stamp', inputs=inputs)
        return stamp

    def _cc_objects_make(self, sources=None, generated=False, generated_headers=None):
        """Generate cc objects build rules in make. """
        vars = {}
        self._setup_make_cc_vars(vars)
        implicit_deps = []
        stamp = self._cc_objects_generated_make_header_files_dependency()
        if stamp:
            implicit_deps.append(stamp)
        secure = self.data.get('secure')
        if secure:
            implicit_deps.append('.phony')

        objs_dir = self._target_file_path() + '.objs'
        
        self._setup_make_cc_flags()
        ...
        for src in srcs:
            obj = '%s.o' % os.path.join(objs_dir, src)
            if secure:
                self._securecc_object_make(obj, src, implicit_deps, vars)
            else:
                rule = 'object' 
                if generated:
                    input = self._target_file_path(src)
                    ...
                    if os.path.exists(path):
                        ...
                        input = self._target_file_path(src)
                self.make_build(obj, rule, inputs=input,
                                 variables=vars)
            objs.append(obj)

        self.data['objs'] = objs

    # add _cc_clean_make() @niuqing
    def _cc_clean_make(self):
        obj = 'clean'
        rule = 'clean'
        self.make_build(obj, rule)
    
    # add _cc_library_make() @niuqing 
    def _static_cc_library_make(self):
        output = self._target_file_path('lib%s.a' % self.name)
        objs = self.data.get('objs', [])
        self.make_build(output, 'static_lib', inputs=objs)
        self._add_default_target_file('a', output)

    def _dynamic_cc_library_make(self):
        output = self._target_file_path('lib%s.so' % self.name)
        ldflags = self._generate_make_link_flags()
        sys_libs, usr_libs = self._make_dynamic_dependencies()
        extra_ldflags = ['-l%s' % lib for lib in sys_libs]
        self._cc_link_make(output, 'solink', deps=usr_libs,
                            ldflags=ldflags, extra_ldflags=extra_ldflags)
        self._add_target_file('so', output)

    def _cc_library_make(self):
        self._static_cc_library_make()
        if self._need_dynamic_library():
            self._dynamic_cc_library_make()

    def _cc_link_make(self, output, rule, deps,
                       ldflags=None, extra_ldflags=None,
                       implicit_deps=None, order_only_deps=None):
        objs = self.data.get('objs', [])
        vars = {}
        ...
        if extra_ldflags:
            vars['extra_ldflags'] = ' '.join(extra_ldflags)
        self.make_build(output, rule,
                         inputs=objs + deps,
                         implicit_deps=implicit_deps,
                         order_only_deps=order_only_deps,
                         variables=vars)

    def _generate_make_link_flags(self):
        """Generate linker flags for cc link. """
        ldflags = []
        extra_linkflags = self.data.get('extra_linkflags')
        if extra_linkflags:
    ...
    def _make_dynamic_dependencies(self):
        """
        Find dynamic dependencies for make build,
        including system libraries and user libraries.
        """
        targets = self.blade.get_build_targets()
        sys_libs, usr_libs = [], []
    ...
    def _make_static_dependencies(self):
        """
        Find static dependencies for make build, including system libraries
        and user libraries.
        User libraries consist of normal libraries and libraries which should
        be linked all symbols within them using whole-archive option of gnu linker.
        """
...
class CcLibrary(CcTarget):
    ...
    # add make_rules in class CcLibrary @niuqing
    def make_rules(self):
        """Generate make build rules for cc object/library. """
        self._check_deprecated_deps()
        if self.type == 'prebuilt_cc_library':
            self._prebuilt_cc_library()
        elif self.srcs:
            self._cc_objects_make()
            self._cc_library_make()
...
def cc_library(name,
    ...
    options = blade.blade.get_options()
    cc_library_profile = options.profile
    # console.info("""%s will no be library, since compiler_env (%s) conflicts with blade profile (%s)"""
    # % (name, compiler_env, options.profile))
    # register targets, register only when compiler_env == profile 
    if compiler_env == None:
        blade.blade.register_target(target)
    elif compiler_env == cc_library_profile:
        blade.blade.register_target(target)
    else:
        console.info("""%s will no be library, since compiler_env (%s) conflicts with blade profile (%s)"""
        % (name, compiler_env, options.profile))
...
class CcBinary(CcTarget):
    ...
    # add _cc_binary_make() @niuqing
    def _cc_binary_make(self, dynamic_link):
        ldflags = self._generate_cc_binary_link_flags(dynamic_link)
        implicit_deps = []
        if dynamic_link:
            sys_libs, usr_libs = self._make_dynamic_dependencies()
        else:
            sys_libs, usr_libs, link_all_symbols_libs = self._make_static_dependencies()
            if link_all_symbols_libs:
                ldflags += self._generate_link_all_symbols_link_flags(link_all_symbols_libs)
                implicit_deps = link_all_symbols_libs
...
#           order_only_deps.append(scm)
        extra_ldflags += ['-l%s' % lib for lib in sys_libs]
        output = self._target_file_path()
        self._cc_link_make(output, 'link', deps=usr_libs,
                            ldflags=ldflags, extra_ldflags=extra_ldflags,
                            implicit_deps=implicit_deps,
                            order_only_deps=order_only_deps)
        self._add_default_target_file('bin', output)

    # add make_rules in class CcBinary @niuqing
    def make_rules(self):
        """Generate make build rules for cc binary/test. """
        self._check_deprecated_deps()
        self._cc_clean_make()
        self._cc_objects_make()
        self._cc_binary_make(self.data['dynamic_link'])
...
class CcPlugin(CcTarget):
    ...
    # add make_rules in class CcPlugin @niuqing 
    def make_rules(self):
        """Generate make build rules for cc plugin. """
        self._check_deprecated_deps()
        self._cc_objects_make()
        ldflags = self._generate_make_link_flags()
        implicit_deps = []
...
def cc_test(name,
    ...
    # get --profile = {debug / release}
    options = blade.blade.get_options()
    cc_test_profile = options.profile
    # register targets, register only when compiler_env == profile 
    if compiler_env == None:
        blade.blade.register_target(cc_test_target)
    elif compiler_env == cc_test_profile:
        blade.blade.register_target(cc_test_target)
    else:
        console.info("""%s will no be test, since compiler_env (%s) conflicts with blade profile (%s)"""
        % (name, compiler_env, options.profile))
