...
# add MakeRulesGenerator @niuqing
from rules_generator import MakeRulesGenerator
...
class Blade(object):
    ...
    def get_build_rules_generator(self):
        ...
        elif self.get_config('global_config')['native_builder'] == 'make':
            return MakeRulesGenerator('Makefile', self.__blade_path, self)
        else:
    ...
    def gen_targets_rules(self):
        ...
        for k in self.__sorted_targets_keys:
            ...
            elif native_builder == 'make':
                blade_object.make_rules()
            else:
            ...
            if rules:
                rules_buf.append('\n')
                rules_buf += rules
        # reverse rules @niuqing
        if native_builder == 'make':
            rules_buf.reverse() 
        return rules_buf
