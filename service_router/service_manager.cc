...
class ServiceManager {
 public:
  ...
  bool createVersion(const service_router::Server& server) {
    std::string key = ConsulNode::getConfigVersionPath(server);
    std::time_t now = common::currentTimeInMs();
    auto result = kv_->setValueSync(key, folly::to<std::string>(now), service_router::FLAGS_router_consul_token);
    if (result) {
      return (*result).getValue();
    }
    return false;
...
int create(ServiceManager* manager) {
  ...
  if (!manager->createVersion(server)) {
    LOG(INFO) << "Create service config version fail.";
    return COMMAND_FAIL;
  }
...
int listAllService(ServiceManager* manager) {
  ...
  for (auto& service_name : service_list) {
    ...
    std::vector<ConsoleTableCell> cols(
        {ConsoleTableCell(service_name), ConsoleTableCell(folly::to<std::string>(config.getTotalShards())),
...
int main(int argc, char* argv[]) {
  ...
  SCOPE_EXIT { service_framework::http::stop(); };
