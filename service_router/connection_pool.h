...
using RSocketClientChannelPtr = std::shared_ptr<apache::thrift::RSocketClientChannel>;
using TAsyncSocket = apache::thrift::async::TAsyncSocket;
 ...
 protected:
  TransportType createConnection(const std::string& addr, uint16_t port) {
    ...
    eventbase_->runInEventBaseThreadAndWait([&addr, port, &conn, this] {
      TAsyncSocket::UniquePtr socket(new TAsyncSocket(eventbase_, addr, port));
  ...
  virtual TransportType createClientConnection(TAsyncSocket::UniquePtr socket) = 0;
...
class ConnectionThread<Http2ClientConnectionIfPtr> : public ConnectionThreadBase<Http2ClientConnectionIfPtr> {
 ...
 protected:
  Http2ClientConnectionIfPtr createClientConnection(TAsyncSocket::UniquePtr socket) {
...
class SafeHeaderClientChannel : public apache::thrift::HeaderClientChannel {
 public:
  explicit SafeHeaderClientChannel(const std::shared_ptr<apache::thrift::async::TAsyncTransport>& transport)
...
class ConnectionThread<HeaderClientChannelPtr> : public ConnectionThreadBase<HeaderClientChannelPtr> {
 ...
 protected:
  HeaderClientChannelPtr createClientConnection(TAsyncSocket::UniquePtr socket) {
    return std::shared_ptr<apache::thrift::HeaderClientChannel>(
        SafeHeaderClientChannel::newSafeChannel(std::move(socket)));
  }
};

// transport rsocket 的特化实现
template <>
class ConnectionThread<RSocketClientChannelPtr> : public ConnectionThreadBase<RSocketClientChannelPtr> {
 public:
  explicit ConnectionThread(folly::EventBase* evb) : ConnectionThreadBase<RSocketClientChannelPtr>(evb) {}
  virtual ~ConnectionThread() = default;

 protected:
  RSocketClientChannelPtr createClientConnection(TAsyncSocket::UniquePtr socket) {
    return std::shared_ptr<apache::thrift::RSocketClientChannel>(
        apache::thrift::RSocketClientChannel::newChannel(std::move(socket)));
  }
};
