...
class ConsulNode {
 public:
  ...
  static const std::string getConfigVersionPath(const Server& server);
...
class ConsulRegistry : public RegistryInterface {
 ...
 private:
  ...
  folly::Synchronized<std::unordered_map<std::string, std::string>> config_versions_;
  ...
  bool checkConfigUpdate(const Server& server);
  void fallback();
