...
constexpr static char CONSUL_REGISTRY_CONFIG_VERSION[] = "version";
...
bool ConsulRegistry::registerServerSync(const Server& updateServer) {
  ...
  auto fut = std::move(result).via(thread_pool.get()).then([this, updateServer](const AsyncConsulResponse& response) {
...
void ConsulRegistry::registerServer(const Server& updateServer) {
  ...
  auto result = getKvClient().setValue(ConsulNode::getNodePath(updateServer), json, FLAGS_router_consul_token);
  result.via(thread_pool.get()).then([this, updateServer](const AsyncConsulResponse& response) {
    auto res = getKvClient().processBool(response);
    if (res && (*res).getValue()) {
      VLOG(4) << "Register server success, result:" << *res;
    } else {
...
void ConsulRegistry::discover(const std::string& service_name) {
  ...
  if (!use_static) {
    ...
    result.via(thread_pool.get()).then([this, server](const AsyncConsulResponse& response) {
...
bool ConsulRegistry::checkConfigUpdate(const Server& server) {
  auto version_res = getKvClient().getValueSync(ConsulNode::getConfigVersionPath(server));
  if (!version_res) {
    VLOG(4) << "Get config version fail, server:" << server;
    return true;
  }
  std::string version = proxygen::Base64::decode(version_res->getValue().getValue(), 0);
  return config_versions_.withULockPtr([&](auto ulock) {
    std::string service_name = server.getServiceName();
    if (ulock->find(service_name) != ulock->end()) {
      if (ulock->at(service_name) == version) {
        // config 没有更新
        return false;
      }
    }
    auto wlock = ulock.moveFromUpgradeToWrite();
    (*wlock)[service_name] = version;
    return true;
  });
}
...
void ConsulRegistry::getConfig(const std::string& service_name) {
  ...
  server.setServiceName(service_name);
  auto thread_pool = callback_thread_pool_.lock();
  if (!thread_pool || !checkConfigUpdate(server)) {
    return;
  }
...
void ConsulRegistry::processConfig(const Server& server, const ConsulValues& values) {
  ...
  for (auto& t : values.getValue()) {
    ...
    std::string vval = proxygen::Base64::decode(t.getValue(), 0);
    VLOG(4) << "key:" << vkey << " value:" << vval;
    if (vkey.find(ConsulNode::getConfigVersionPath(server)) != std::string::npos) {
      continue;
    }
...
const std::string ConsulNode::getConfigVersionPath(const Server& server) {
  return getConfigPath(server) + CONSUL_REGISTRY_CONFIG_VERSION;
}
