...
template<typename T> void PredictorService::send_req_example_snapshot(const T& req) {
  if (FLAGS_example_snapshot_frequency > 0 &&
    ...
    if (!common::serializeThriftObjToCompact(&serialize_str, req)) {
      FB_LOG_EVERY_MS(ERROR, 2000) << "Err: serializeThriftObjToJson failed!";
    } else {
...
void PredictorService::calculateBatchVector(CalculateBatchVectorResponses& calculate_batch_vector_responses,
  ...
  const MetricTagsMap tag_map{{TAG_REQ_TYPE, REQ_TYPE_CALCULATE_BATCH_VECTOR}};
  metrics::Timer timer(util::buildTimers(TIME_CONSUMING, tag_map));

  model_manager_->calculateBatchVector(&calculate_batch_vector_responses,
                                      std::move(calculate_batch_vector_requests_ptr));
}
